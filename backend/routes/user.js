...
router.post("/auth/login", max.limiter, userCtrl.login);                //Connexion d'un compte utilisateur existant.
router.get("/accounts/:id", auth, userCtrl.getOneUser);                 //Récupération des informations d'un compte utilisateur par son id.
router.get("/accounts", auth, userCtrl.getAllUsers);                    //Récupération des informations de tous les comptes utilisateurs.
router.put("/accounts/:id", auth, userCtrl.modifyUser);                 //Mise à jour d'un compte utilisateur.
